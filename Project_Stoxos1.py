...
symbool = cv2.imread('Images/image2111.png',0)
w, h = symbool.shape[::-1]


img = prev
cv2.imwrite('Output/Stoxos/prevImg.png',img.copy())
...
cv2.imwrite('Output/Stoxos/newPrevImg.png',newPrevImg.copy())
...
cv2.destroyWindow('frame')
img = after
cv2.imwrite('Output/Stoxos/afterImg.png',img)
img_gray = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
found = None
for scale in np.linspace(0.01, 1.0, 20)[::-1]:
    resized = cv2.resize(img_gray, None, fx=scale, fy=scale)
...
cv2.imwrite('Output/Stoxos/newAfterImg.png',newAfterImg)

h,w,_ = newPrevImg.shape
newAfterImg = cv2.resize(newAfterImg,(w,h))
cv2.imshow("TELIKO TELIKO newAfterImg",newAfterImg)
cv2.imwrite('Output/Stoxos/FinalAfterImg.png',newAfterImg)
...
cv2.waitKey(0)
cv2.imwrite('Output/Stoxos/shots.png',sure_)
...
_,target_contours,_ = cv2.findContours(blurA,cv2.RETR_EXTERNAL,cv2.CHAIN_APPROX_NONE)#~~~~~~~~~
...
temp = prev_cp.copy()    
cv2.drawContours(temp,target_contours,0,(0,255,255),3)
...
cv2.imshow("target with contours",prev_cp)
cv2.waitKey(0)
cv2.imwrite('Output/Stoxos/TargetWithVirtualShots.png',prev_cp)
...
for i in range(0,a-1):
    print('i = ' + str(i+1))
    #print("i=" + str(i) + " " + str(centers[i]))
    for j in range (0,numOfRings):
    	#print("j=" + str(j) + " " + str(TargetClasses[j]) )
...
output.close()
f = open("Output/Stoxos/output.txt",'w')
